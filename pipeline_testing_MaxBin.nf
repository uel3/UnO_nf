...
workflow {
    ...
    max_bin_ch = MAXBIN2_BIN( megahit_assembly_ch.megahit_contigs )
...
process BOWTIE2_MAP_READS{
    ...
    """
    bowtie2 -p 8 -x ${idx} -q -1 ${reads_trimmed[0]} -2 ${reads_trimmed[1]} --no-unal -S ${sample_id}.sam
    """
        //samtools view -@ "2" -bS -o ${sample_id}.bam - 
...
/*
 * Samtools preperation of Megahit assembled reads for Binning
 */
process MAXBIN2_BIN{
    ...
    output:
    path( "${assembly}_maxbin2*.fastq" ), emit: maxbin2_bins 

    script:
    """
    run_MaxBin.pl -contig /scicomp/home-pure/uel3/metagenome_practice/megaout/filteredunalignedcontigs.fasta -out maxbin -reads unmapped_1.fastq -reads2 unmapped_2.fastq
    """
    stub:
    """
